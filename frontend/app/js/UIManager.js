...
window.UIManager = {
    init: function() {
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (tabName === 'chart' && window.ChartAnalytics && window.ChartAnalytics.chart) {
        ...
        document.getElementById('play-btn').addEventListener('click', () => {
            if (!window.SIM_STATE.isRunning) {
                ...
                window.SIM_STATE.lastError = window.SIM_STATE.setPoint - window.SIM_STATE.position;
    ...
    updateDisturbance: function(disturbance) {
        ...
        if (barEl) {
            ...
            barEl.style.background = Math.abs(disturbance) > maxForce * 0.6 
                ? '#ff4444' 
                : '#FFB703';
    ...
    updateMetrics: function(riseTime, overshoot, settlingTime, error, vel) {
        ...
        if (riseEl) riseEl.textContent = riseTime ? riseTime.toFixed(2) + 's' : '--';
